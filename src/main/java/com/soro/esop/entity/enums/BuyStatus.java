...
public enum BuyStatus implements BaseEnum {
    ...
    W ("W", Name.builder().ko("매수대기").en("Wait").zh("대기").ja("대기").build()),     // ordinal: 1
    I ("I", Name.builder().ko("매수수량입력").en("Input").zh("입력").ja("입력").build()),    // ordinal: 2
    C ("C", Name.builder().ko("예탁진행").en("Confirm").zh("승인").ja("승인").build()),  // ordinal: 3
    F ("F", Name.builder().ko("매수완료").en("Finished").zh("완료").ja("완료").build()); // ordinal: 4
